...
;( function( window ) {
	 ...
	 * FForm function
	 */
	function FForm( el, options , completed) {
		this.el = el;
		this.options = extend( {}, this.options );
  		extend( this.options, options );
  		this._init();
  		for( var i = 0; i < completed.questions.length; ++i ) {
  			console.log(completed.questions[i])
				$("#"+completed.questions[i]).attr('checked','true');
			}
  		this._showField(completed.current)
	}
	...
	FForm.prototype._addControls = function() {
		...
		this.ctrlContinue = createElement( 'button',{ cName : 'fs-continue', inner : '保存测试', appendTo : this.ctrls } );
	 ...
	 */
	FForm.prototype._save = function( backto ) {
		console.log("save")
		var ops = $('#myform').serialize()
		ops+="&current="+this.current
        $.ajax({
            //几个参数需要注意一下
                type: "POST",//方法类型
